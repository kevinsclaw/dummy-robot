...
Run `pnpm nx build @my-project/infra [--skip-nx-cache]` to build the application.

## Running unit tests

Run `pnpm nx test @my-project/infra` to execute the unit tests via Vitest.

### Updating snapshots

To update snapshots, run the following command:

`pnpm nx test @my-project/infra --configuration=update-snapshot`

## Run lint

Run `pnpm nx lint @my-project/infra`

### Fixable issues

You can also automatically fix some lint errors by running the following command:

`pnpm nx lint @my-project/infra --configuration=fix`

## Deploy to AWS

### Deploy all Stacks

Run `pnpm nx deploy @my-project/infra --all`

### Deploy a single Stack

Run `pnpm nx deploy @my-project/infra [stackName]`
...
Run `pnpm nx deploy @my-project/infra --hotswap --all`
...
```typescript
import { suppressRules } from ':my-project/common-constructs';
